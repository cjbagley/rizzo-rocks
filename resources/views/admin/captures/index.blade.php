            ...
            <x-admin-card>
                <div class="game-info-wrapper">
                    ...
                    @else
                        <video width="160" controls>
                            ...
                            <source src="{{ $capture->url }}" type="video/webm"/>
                        </video>
                    @endif
                ...
                <div class="btn-container">
                    <x-secondary-button-link
                        href="{{ route('captures.edit', ['game' => $game, 'capture' => $capture]) }}">Edit
                    </x-secondary-button-link>
                    <x-danger-button x-data=""
                                     x-on:click.prevent="$dispatch('open-modal', 'confirm-deletion-{{$capture->id}}')">{{ __('Delete') }}</x-danger-button>
                </div>
                <x-modal name="confirm-deletion-{{$capture->id}}" :show="$errors->gameDeletion->isNotEmpty()" focusable>
                    <form method="post"
                          action="{{ route('captures.destroy', ['game' => $game, 'capture' => $capture]) }}">
                        @csrf
                        @method('delete')
