<script>
	...
	function toggleSiteTheme() {
		let newTheme = getAppliedTheme() == LIGHT_THEME ? DARK_THEME : LIGHT_THEME;
	...
	function applySiteTheme(theme) {
		if (theme == LIGHT_THEME) {
			...
			document.querySelectorAll('.dark-mode-icon').forEach((el) => el.classList.remove('hidden'));
			document.querySelectorAll('.light-mode-icon').forEach((el) => el.classList.add('hidden'));
		} else {
			document.getElementsByTagName('html')[0].classList.add('dark');
			document.querySelectorAll('.dark-mode-icon').forEach((el) => el.classList.add('hidden'));
			document.querySelectorAll('.light-mode-icon').forEach((el) => el.classList.remove('hidden'));
		}
	}
...
<button id="dark-mode-toggle" on:click={toggleSiteTheme}>
	<svg
		...
		height="30"
		fill="currentColor"
		class="dark-mode-icon hidden bi bi-brightness-high-fill text-white"
		viewBox="0 0 16 16"
	>
		<path
	...
	<svg
		...
		height="30"
		fill="currentColor"
		class="light-mode-icon hidden bi bi-moon-fill text-black"
		viewBox="0 0 16 16"
	>
		<path
